...
void rtl8169_set_high(struct rtl8169_private *tp, u8 reg_bit) 
{
	reg_bit|=RTL_R8(tp, Cfg9346);
	RTL_W8(tp, Cfg9346, reg_bit); 
}

void rtl8169_set_low(struct rtl8169_private *tp, u8 reg_bit)
{
	reg_bit=RTL_R8(tp,Cfg9346) & ~reg_bit;
	RTL_W8(tp, Cfg9346, reg_bit);
}

void rtl8169_eeprom_clock(struct rtl8169_private *tp, int cycles) 
{
	while (cycles>0) {
		rtl8169_set_high(tp, EESK);
		udelay(EEPROM_DELAY);
		rtl8169_set_low(tp, EESK);
		udelay(EEPROM_DELAY);
		cycles--;
	}
}

void send_start_bit(struct rtl8169_private *tp) 
{
	rtl8169_set_high(tp,EEDI);
	rtl8169_eeprom_clock(tp, 1);
}

void erase_write_enable(struct rtl8169_private *tp) 
{
	rtl8169_set_high(tp, EECS);
	send_start_bit(tp);
	rtl8169_set_low(tp, EEDI);
	rtl8169_eeprom_clock(tp, 2);
	rtl8169_set_high(tp, EEDI);
	rtl8169_eeprom_clock(tp, 8);
}

void erase_write_disable(struct rtl8169_private *tp) 
{
	rtl8169_set_high(tp, EECS);
	send_start_bit(tp);
	rtl8169_set_low(tp, EEDI);
	rtl8169_eeprom_clock(tp, 10);
}

void send_address(struct rtl8169_private *tp, u8 address) 
{
	int i;
	for (i=0; i<8; i++)
	{
...
void prepare_eeprom_for_read(struct rtl8169_private *tp) 
{
	rtl8169_set_high(tp, EEDI);
	rtl8169_eeprom_clock(tp, 1);
	rtl8169_set_low(tp, EEDI);
...
void prepare_eeprom_for_write(struct rtl8169_private *tp) 
{
	rtl8169_set_low(tp, EEDI);
	rtl8169_eeprom_clock(tp, 1);
	rtl8169_set_high(tp, EEDI);
...
void prepare_eeprom_for_erase_word(struct rtl8169_private *tp) 
{
	rtl8169_set_high(tp, EEDI);
	rtl8169_eeprom_clock(tp, 2);
}

void rtl8169_eeprom_erase_all(struct rtl8169_private *tp) 
{
	rtl8169_set_high(tp, EECS);
	rtl8169_set_low(tp, EEDI);
	rtl8169_eeprom_clock(tp, 2);
	/*this is a hack*/
	send_address(tp, 0x80);
	rtl8169_set_low(tp, EECS);
}

void send_data(struct rtl8169_private *tp, u16 data, bool even) 
{
	int i;
	/*we use different masks to send a single byte or 2 bytes,
	 * here we are sending 2 bytes*/
	if (even) {
		for (i=0; i < 16; i++)
		{
			if (data & 0x8000)
	...
	/*we then send 8 more zeros so the write adds up to 2 bytes*/
		rtl8169_set_low(tp, EEDI);
		rtl8169_eeprom_clock(tp, 8);
	}
}

void rtl8169_eeprom_write_all(struct rtl8169_private *tp, u16 data) 
{
	rtl8169_set_high(tp, EECS);
	rtl8169_set_low(tp, EEDI);
	rtl8169_eeprom_clock(tp, 2);
	/*Another hack*/
	send_address(tp, 0x40);
	send_data(tp, data, 1);
}


void write_16bit_serial_data(struct rtl8169_private *tp, u16 data, 
		u8 address, bool last, bool even) 
{
	/*Enable chip select*/
	rtl8169_set_high(tp, EECS);

	/*test to make sure eeprom is done writing previous word*/
	while (!(RTL_R8(tp, Cfg9346) & EEDO))
		rtl8169_eeprom_clock(tp, 1);	
	send_start_bit(tp);
	prepare_eeprom_for_write(tp);
	send_address(tp, address);
	
	/*It is recommended to disable erase/write operations 
	  following the last erase/write operation*/
	
	send_data(tp, data, even);
	if (last)
		erase_write_disable(tp);
	rtl8169_set_low(tp, EECS);
}

u16 read_16bit_serial_data(struct rtl8169_private *tp, u8 address) 
{
	int i;
	u16 value=0;
	rtl8169_set_high(tp, EECS);
	send_start_bit(tp);
	prepare_eeprom_for_read(tp);
	send_address(tp, address);
	/*17 is used instead of 16 because the eeprom sends out a dummy 
	 * zero before every read. The zero will eventually be shifted out of value*/
	for (i=0; i<16; i++)
	{
		if (RTL_R8(tp,Cfg9346) & EEDO) 
			value |=0b1;
		value <<=1;
	       rtl8169_eeprom_clock(tp, 1);	
	}
...
static int rtl8169_get_eeprom(struct net_device *netdev,
					struct ethtool_eeprom *ee,
					u8 *data)
{
	struct rtl8169_private *tp = netdev_priv(netdev);
	u8 address = ee->offset;
	u16 value=0;
	int i=0, length = ee->len;
	if (length < 0)
		return -EINVAL;
	switch(tp->eepromtype) {
	case EEPROM_9356:
		length = min(length, 0xff-address);
		break;
	case EEPROM_9346:
		length = min(length, 0x3f-address);
		break;
	case NO_EEPROM:
		return -EOPNOTSUPP;
	}
	...
	while (i <= length) {
		value = read_16bit_serial_data(tp, address);
	       data[i]=value;
		i+=2;
	       address++;
	}
	/*if an odd number of bytes were read, i = bytes_read - 1*/
	ee->len = i;
	return i;
}

static int rtl8169_set_eeprom(struct net_device *netdev, 
		struct ethtool_eeprom *ee, u8 *buf) 
{
	...
	struct rtl8169_private *tp = netdev_priv(netdev);
	int size =  ee->len;
	erase_write_enable(tp);
	/*decrease by 2 because we cast pointer to char(1 byte) to pointer to u16(2 bytes)*/
	while(size > 0)
	{
		int i = 0;
		data = buf[i];
		if (size<=2) {
			last=1;
			/*the eeprom writes only 16 bits at a time but the last item 
			 * in the buffer may be a byte, this checks for that*/
			if (size==1)
				even=0; }
		write_16bit_serial_data(tp, data, ee->offset, last, even);
		ee->offset++;
		/*the last increment may be past the buffer incase there were 
		 * an odd number of bytes but it will have no problem 
		 * since the pointer will not be dereferenced*/
		i+=2;
		size-=2;
	}
...
void set_eepromtype(struct rtl8169_private *tp)
{
	/* I couldn't find another way to test for the presence of
	 * an eeprom so I try to read the first address of the
	 * eeprom for the code word 8192h. This is a very crude 
	 * method */
	if (read_16bit_serial_data(tp, 0x0) & 0x8192) {
		tp->eepromtype=(RTL_R8(tp,RxConfig) & 0x40) ? 
			EEPROM_9356 : EEPROM_9346;
	}
	else
		tp->eepromtype=NO_EEPROM;
}

static int rtl8169_get_eeprom_len(struct net_device *netdev)
{
	struct rtl8169_private *tp = netdev_priv(netdev);
	switch(tp->eepromtype) {
		case EEPROM_9356:
